...
BASE_HEALTH = settings.BASE_HEALTH

class Enemy:
    def __init__(self, e_area:list[list[int]], start_tiles:list[int, int], rect:pygame.rect.RectType, seconds:int):
        ...
        for idx, tile in enumerate(start_tiles):
            ...
            self.enemyTimer[-1]["rect"].center = (px, py)
            self.timer = 0
        self.DIR_LOOKUP = {
            1: (0, ENEMY_SPEED),
            2: (-ENEMY_SPEED, 0),
    ...
    def move(self, screen_rect: pygame.rect.RectType):
        self.timer += 1
        ...
        for idx, e in enumerate(self.enemies):
            ...
            rect = e["rect"]
            dir = e["direction"]
            e["tile"][0] = rect.centerx // TILE_SIZE
            e["tile"][1] = rect.centery // TILE_SIZE
            ...
            prevLoc = rect.center
            rect.move_ip(dir)
            if rect.center == prevLoc:
                remove.append(idx)
            e["rect"] = rect
            e["direction"] = dir
            e["timer"] -= ENEMY_SPEED
        for i in remove: self.enemies.pop(i)
