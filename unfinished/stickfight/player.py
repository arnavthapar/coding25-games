...
class Player:
    def __init__(self):
        ...
        self.anim = 7
        for i in range(7):
            self.images.append(pygame.image.load("Images/Idle" + str(i+1) + ".svg"))
        for i in range(7):
            self.images.append(pygame.image.load("Images/Run" + str(i+1) + ".svg"))
        self.x_velocity = 0
        self.max_velocity = 4
        self.y_velocity = 0
        self.direction = 0
    ...
    def move(self, area):
        ...
        # Move on the x axis.
        self.x_velocity += self.direction/20
        ...
        self.rect.y += round(self.y_velocity)
    ...
    def blitme(self, screen, area):
        ...
        if self.anim >= 13.5:
            self.anim = 7
        self.move(area) # Move the player.
        # Draw the player.
        if self.direction == -1: screen.blit(pygame.transform.flip(self.images[round(self.anim)], True, False),(self.rect.x, self.rect.y))
        elif self.direction == 1: screen.blit(self.images[round(self.anim)],(self.rect.x, self.rect.y))
        else: screen.blit(self.images[round(self.anim) - 7], (self.rect.x, self.rect.y))
